...
export class WithdrawService {
    ...
    async createWithdraw(withdrawData: IReqCreateWithdraw) {
        ...
        let savedWithdraw;
        if (existingWithdraw) {
            if (existingWithdraw.data_time.getDay() === new Date().getDay()) {
        ...
        if (existingWithdraw?.data_time.getDay() === new Date().getDay()) {
            const withdrawDetailData = await this.withdrawDetailRepository.findOne({ where: { withdraw_id: savedWithdraw.id, ice_id: withdrawData.product_id } });
